    ...
    <section className="space-y-3">
      ...
      {roundsApprovedFirstTryTotal + roundsReturnedTotal > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <BarChart
            data={[
              { label: "Aprobadas", value: roundsApprovedFirstTryTotal, colorClass: "progress-fill-emerald-performance" },
              { label: "Devueltas", value: roundsReturnedTotal, colorClass: "progress-fill-red-performance" },
            ]}
          />
        </div>
      )}
...
export default async function PerformancePage({
  ...
  const delayRanked = performance
    ...
    .map((p) => ({ label: p.userName, value: p.totalDelayDays, colorClass: "progress-fill-red-performance" }))
    .sort((a, b) => b.value - a.value);
  ...
  const contributionBars = workload
    ...
    .map((w) => ({
      label: w.userName,
      value: w.percent,
      colorClass: w.percent === 0 ? "bg-slate-300" : "progress-fill-teal-performance",
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <BarChart data={onTimeRanked.map((r) => ({ ...r, colorClass: "progress-fill-emerald-performance" }))} valueSuffix="%" />
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
