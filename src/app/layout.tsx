  ...
  return (
    ...
    >
      <body className="min-h-full flex flex-col">
        ...
        />
        <script
          // Ctrl+B o Ctrl+N (N de negrita) / Cmd en un campo de texto: envuelve la selección en *asteriscos*
          ...
          dangerouslySetInnerHTML={{
            __html: `document.addEventListener('keydown', function (e) { if (e.defaultPrevented || !(e.ctrlKey || e.metaKey) || e.shiftKey || e.altKey || (e.key.toLowerCase() !== 'b' && e.key.toLowerCase() !== 'n')) return; var t = e.target; if (!(t instanceof HTMLElement) || t.isContentEditable) return; if (!(t.tagName === 'TEXTAREA' || (t.tagName === 'INPUT' && (t.type === 'text' || t.type === 'search')))) return; e.preventDefault(); var v = t.value, a = t.selectionStart, b = t.selectionEnd; while (a < b && /\\s/.test(v[a])) a++; while (b > a && /\\s/.test(v[b - 1])) b--; var sel = v.slice(a, b), from = a, to = b, out; if (sel.length > 1 && sel[0] === '*' && sel[sel.length - 1] === '*') { out = sel.slice(1, -1); } else if (a > 0 && v[a - 1] === '*' && v[b] === '*') { from = a - 1; to = b + 1; out = sel; } else { out = '*' + sel + '*'; } t.setSelectionRange(from, to); if (!document.execCommand('insertText', false, out)) { var proto = t.tagName === 'TEXTAREA' ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype; Object.getOwnPropertyDescriptor(proto, 'value').set.call(t, v.slice(0, from) + out + v.slice(to)); t.dispatchEvent(new Event('input', { bubbles: true })); } var c = from + out.length - (sel === '' && out === '**' ? 1 : 0); if (sel === '' ) c = from + 1; t.setSelectionRange(sel === '' ? c : from, sel === '' ? c : from + out.length); });`,
          }}
        />
      </body>
    </html>
